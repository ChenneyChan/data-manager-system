...
namespace ABBDataManagerSystem.Pages
{
    ...
    public partial class JinYuan20W : UserControl, ICloseable
    {
        ...
        private void UpdateRealTimePanelDisplay(JinYuan20WCollector.JinYuan20WPacketInfo packet)
        {
            ...
            {
                needRecordTempRise = packet.tempRaiseTime != lastPacket.tempRaiseTime && packet.tempRaiseTime.Trim() != ":"
                    && !(packet.ch1RealTimeCurrent == 0 && packet.ch2RealTimeCurrent == 0);
            }
            lastPacket = packet;
        ...
        private void ToolgeTestingStatus()
        {
            ...
            if (Collector != null && !Configs.Configs.IsEnableTesting)
            {
                ...
                Dispatcher.Invoke(() =>
                {
                    if (SelectedTesting == TestType20W.CommonTest) //常规测试
                    {
                        panelConfig.IsEnabled = true;
                        panelTestChoice.IsEnabled = true;
                        TimerSecond.Stop();
